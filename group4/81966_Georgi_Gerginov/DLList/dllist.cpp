...
DLList::Node::Node(const int& _value, Node* _next = nullptr, Node* _prev = nullptr)
    : value{_value} {}
...
DLList::DLList()
    : DLList{0, nullptr, nullptr} {}

DLList::DLList(const size_t& length = 0, Node* start = nullptr, Node* end = nullptr)
    : m_length{length}, m_start{start}, m_end{end} {}
...
void DLList::push_front(const int& new_elem) {
    m_length++;
    m_start = new Node{new_elem, m_start};
...
void DLList::push_back(const int& new_elem) {
    m_length++;
    m_end = new Node{new_elem, nullptr, m_end};
